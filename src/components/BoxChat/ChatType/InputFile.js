...
const InputFile = () => {
  ...
  const handleOnChangeFile = (event) => {
    ...
    } else if (targetFile && /^audio\/[a-z0-9]/.test(targetFile.type)) {
      handleSendFile(event.target.files, MessageTypes.AUDIO);
    } else {
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center", pb: 1, pl: 1 }}>
      <label htmlFor="ic-camera">
        <Typography
          className={classes.IconCamera + " " + classes.icon}
        ></Typography>
      </label>
      <input
        style={{ display: "none" }}
      ...
      <label htmlFor="ic-img">
        <Typography
          className={classes.iconPicture + " " + classes.icon}
        ></Typography>
      </label>
      <input
        style={{ display: "none" }}
