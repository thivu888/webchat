...
import { updateFocusRight, updateShowAddGroup, updateTargetContentRight, updateUserInfo } from '../../actions/Main'
...
const Container = () => {
  ...
  const CallAPI = (value) => {
    setLoading(true)
    UserService.findUsers(0, 10, value).then(res => setData([...res.data])).then(() => setLoading(false))
  }
  const handleCallAPI = _.debounce((value) => CallAPI(value), 500)

  const getItems = () => {
    return data.map(user => <AddFriendItem key={user._id} user={user} setTags={setTags} />)
  }
  const getTags = () => {
    return tags.map(item => <Chip icon={<FaceIcon />} key={item._id} id={item._id} user={item} label={item.username} onDelete={() => ondeleteTag(item._id)} variant="outlined" />)
  }
  ...
  return (
    <Backdrop sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }} open={!!showFindAddGroup} setData={setData} >
      <Box sx={{ width: 360, height: 615, background: '#fff', borderRadius: 3, p: 1, zIndex: (theme) => theme.zIndex.drawer + 2 }} >
        ...
        <Box sx={{ float: 'right', mt: 2 }}>
          <Button variant="contained" disabled={tags.length > 0 ? false : true} onClick={onSubmit}>Bắt đầu</Button>
        </Box>
      </Box>
    </Backdrop>
  )
