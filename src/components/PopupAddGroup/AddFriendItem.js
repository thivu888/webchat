...
export default function AddFriendItem({user,setTags}) {
    ...
    const handleAddToTag = (user) => {
        setTags(tag => [...tag?.filter(item => item._id != user._id),{...user}])
    }

    const me = storage.getUserInfo()
    return (
        <Box className={classes.item_wraper} sx={{display: 'flex', alignItems:'center',borderRadius: '16px', height: '50px !important'}} onClick={() => handleAddToTag(user)}>
