...
const UserService = {
  ...
  async cancelAddFriend(userId, friendId) {
    return HttpRequest.delete(`/friends/cancel/${userId}/${friendId}`).then(
      (res) => res
    );
  },

  async acceptFriend(data) {
    return HttpRequest.put(`/friends/accept`, data).then((res) => res);
  },

  async checkIsFriend(userId, friendId) {
    return HttpRequest.post(`/users/friend/${userId}/${friendId}`).then(
      (res) => res
    );
  },

  async getRequestFriends(id) {
    return HttpRequest.get(`/users/friend-request/${id}`).then((res) => res);
  },

  async getSentRequestFriends(id) {
    return HttpRequest.get(`/users/myrequest/${id}`).then((res) => res);
  },

  async getRecommendAddFriend(id) {
    return HttpRequest.get(`/users/recommend/${id}`).then((res) => res);
  },

  async findUsers(page = 0, limit = 10, search = " ") {
    return HttpRequest.get(`/users/${page}/${limit}?search=${search}`).then(
      (res) => res
    );
  },
